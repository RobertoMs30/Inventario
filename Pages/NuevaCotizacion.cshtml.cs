...
public class NuevaCotizacionModel : PageBase
{
    private readonly IConfiguration _config;
    private readonly IHttpClientFactory _httpFactory;

    public NuevaCotizacionModel(IConfiguration config, IHttpClientFactory httpFactory)
    {
        _config = config;
    ...
    [BindProperty]
    public CotizacionForm Form { get; set; } = new();
    ...
    // ── Autocomplete: buscar clientes en cat_clientes ────────
    ...
    // ── Autocomplete: buscar en catálogo de materiales ───────
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        // Limpiar errores automáticos del framework
        ...
        // ── Validar que haya al menos un ítem (no-partida) ──
        ...
        await using var cmd = new SqlCommand("dbo.sp_CrearCotizacion", conn);
        ...
        // El SP retorna el id de la nueva cotización
        var result = await cmd.ExecuteScalarAsync();
        int nuevaId = result != null ? Convert.ToInt32(result) : 0;

        return RedirectToPage("/Cotizaciones", new { saved = 1 });
    }
